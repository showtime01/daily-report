...
export default function AdminCustomerEditPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/v1/customers/${id}`).then((r) => {
        ...
        return r.json() as Promise<{
          ...
          sales: Array<{ user_id: number; user_name: string; department: string | null; assigned_at: string }>
        }>
      }),
      fetch('/api/v1/users?role=sales').then((r) => r.ok ? r.json() as Promise<SalesUser[]> : []),
    ]).then(([customer, users]) => {
      if (customer) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        sales: salesList.map((s) => ({ user_id: s.user_id, assigned_at: s.assigned_at })),
      }
  ...
  if (notFound) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <p className="text-lg font-semibold text-gray-700">顧客が見つかりません</p>
          <button onClick={() => router.push('/admin/customers')} className="mt-4 text-sm text-blue-600 underline">
            一覧に戻る
          </button>
        </div>
      </div>
  ...
  if (!currentUser || loadingData) return null
